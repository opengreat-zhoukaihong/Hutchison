...
<%
 String closePage="MatchSearch.jsp";
  ...
  else
  {
   ...
   if (url!=null && url.equalsIgnoreCase("matchStatu")) 
   {
     ...
     else
     {
      ...
      if(!mm.updateMatchStatus(matchNum,deliveryRequestNum,truckCapacityNum,constant.MATCH_STATUS_CONFIRM))
...
function doSubmit()
{
 if(!isDate(matchSearch.deliveryDate.value) && matchSearch.deliveryDate.value!=0)
 {
   ...
   return;
    ...
    matchSearch.action="MatchSearch.jsp"
    matchSearch.submit();
}
function doPost(matchNum,deliveryNo,truckNo)
{
    matchSearch.action="MatchSearch.jsp?url=matchStatu&matchNum="+matchNum+"&deliveryRequestNum="+deliveryNo+"&truckCapacityNum="+truckNo;
    matchSearch.submit();
...
<form name="matchSearch" method="POST" action="MatchSearch.jsp">
...
%>
  ...
  <tr>
    ...
    <td width="10%">
    <%
    if(mcrm.getMatchStatus().equals(constant.MATCH_STATUS_MATCHED))
     {
    %>
      <input type="button" name="button" onClick="JavaScript:doPost('<%=mcrm.getMatchNum()%>','<%=mcrm.getDeliveryRequestNum()%>','<%=mcrm.getTruckCapacityNum()%>')" value="確認">
    <%
     }
    else {
    %>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" height="22" align="center">
  <tr>
    <td width="28%" class="unnamed1">
          ...
          {
            %>
      <div align="left"><a href="MatchSearch.jsp?type=F&init=tmd"><font color="#003366">第一頁</font></a> <a href="MatchSearch.jsp?type=P&init=tmd"><font color="#003366">上一頁</font></a>
          <%
          }
          if (pagLt.getPageNo() < pagLt.getPageSum())
          {
            %>
        <a href="MatchSearch.jsp?type=N&init=tmd"><font color="#003366">下一頁</font></a> <a href="MatchSearch.jsp?type=E&init=tmd"><font color="#003366">最後一頁</font></a>
          <%
          }
          %>
      </div>
