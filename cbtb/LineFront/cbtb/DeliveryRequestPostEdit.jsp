...
function cargoIssuer_onchange() {
  ...
  add.action = "DeliveryRequestPostEdit.jsp#3";
  add.submit();
}
...
%>
  <tr> 
      <td>出發地城市：</td>
      <td> 
          <%= dbList.getCityDesc(deliveryRequestMode.getOriginCityId(),sMarkLanguage) %>
          <input type="hidden" name="originCityId" value="<%= deliveryRequestMode.getOriginCityId() %>">
      </td>
      <td>目的地城市：</td>
      <td> 
         <%= dbList.getCityDesc(deliveryRequestMode.getDestinationCityId(),sMarkLanguage) %>
         <input type="hidden" name="destinationCityId" value="<%= deliveryRequestMode.getDestinationCityId() %>">
      </td>
    </tr>
    <tr> 
      <td>發貨日期：</td>
      <td> 
         <%= deliveryRequestMode.getDeliveryDate().toString().substring(0,10)  %>
         <input type="hidden" name="deliveryDate" value="<%= deliveryRequestMode.getDeliveryDate().toString().substring(0,10)  %>">
      </td>
      <td>&nbsp;</td>
      <td>&nbsp; </td>
    </tr>
    <tr> 
      <td>發貨時閒：</td>
      <td> 
         <%= dbList.getDeliveryTimeDesc(deliveryRequestMode.getTimeId(),sMarkLanguage) %>
         <input type="hidden" name="deliveryTimeId" value="<%= deliveryRequestMode.getTimeId()  %>">
      </td>
      <td>發貨時閒段：</td>
      <td> 
         <%= dbList.getDeliveryTimeSlotDesc(deliveryRequestMode.getTimeSlotId(),sMarkLanguage) %>
         <input type="hidden" name="deliveryTimeSlot" value="<%= deliveryRequestMode.getTimeSlotId() %>">
      </td>
    </tr>
    <tr> 
      <td>貨物種類：</td>
      ...
      <td>要求貨櫃檯種類：</td>
      <td> 
         <%= dbList.getContainerTypeDesc(deliveryRequestMode.getContainerTypeId(),sMarkLanguage) %>
         <input type="hidden" name="requestContainerId" value="<%= deliveryRequestMode.getContainerTypeId() %>">
      </td>
    </tr>
    <tr> 
      <td height="20">數量：</td>
    ...
    <tr> 
      <td>重量：</td>
      <td> 
        <input type="text" maxlength="30" size="20" name="cargoWeight" value="<%= deliveryRequestMode.getCargoWeight() %>" javascript  onchange="return cargoWeight_onchange()"><font color=#ff0033>*</font>
      </td>
      <td>&nbsp;</td>
      <td>&nbsp; </td>
    </tr>
    <tr> 
      <td>航運公司：</td>
      <td> 
         <%= dbList.getShippingLineDesc(deliveryRequestMode.getShippingLineId(),sMarkLanguage) %>
         <input type="hidden" name="shippingLineId" value="<%= deliveryRequestMode.getShippingLineId() %>">
      </td>
      <td>落貨紙號碼/提貨單號碼：</td>
      <td> 
...
%>
    <tr> 
      <td>出發地城市：</td>
      <td> 
          <%= dbList.getCityDesc(request.getParameter("originCityId"),sMarkLanguage) %>
          <input type="hidden" name="originCityId" value="<%= deliveryRequestMode.getOriginCityId() %>">
      </td>
      <td>目的地城市：</td>
      <td> 
          <%= dbList.getCityDesc(request.getParameter("destinationCityId"),sMarkLanguage) %>
          <input type="hidden" name="destinationCityId" value="<%= deliveryRequestMode.getDestinationCityId() %>">
      </td>
    </tr>
    <tr> 
