...
function validDate(strDate,obj){
   ...
   if(deliveryYear < now.getYear()){
      alert("送貨時間的年份要等於 " + now.getYear() + " 年!");
      obj.focus();
      return false;
   }
   else{ //es4
        if((deliveryYear > now.getYear())){
           if((deliveryYear > now.getYear()+1) && (now.getMonth()+1 == 12)){
               alert("送貨時間的年份 '" + deliveryYear + "' 要小於或等於 " + (now.getYear()+1) + " 年!");
               obj.focus();
               return false;
           }
           else{
                if(deliveryYear > now.getYear()+1){
                   alert("送貨時間的年份 '" + deliveryYear + "' 要等於 " + now.getYear() + " 年!");
                   obj.focus();
                   return false;
                }
           }       
           if((deliveryYear = now.getYear()+1) && (now.getMonth()+1 == 12)){
              if(deliveryMonth > 1){
                alert("送貨時間 '" + inputDate  + "' 要小於 '" + deliveryYear + "-01-" + now.getDate() + "' !");
                obj.focus();
                return false;
              }
              else{ 
                   if(deliveryDate > now.getDate()){
                      alert("送貨時間 '" + inputDate  + "' 不能大於  '" + deliveryYear + "-01-" + now.getDate() + "' !");
                      obj.focus();
                      return false;
                   }
             }
          ...
          if(deliveryMonth > now.getMonth()+2){
             alert("送貨時間的月份要小於或等於 " + (now.getMonth()+2) + " 月份!");
             obj.focus();
             return false;
          }
          else{ //es2
               if(deliveryMonth < now.getMonth()+1){
                  alert("送貨時間的月份不能小於 "+ now.getYear() + " 年 " + (now.getMonth()+1) + " 月份!");
                  obj.focus();
                  return false;
               } 
               else{   //es1
                    if((deliveryMonth == now.getMonth()+1) && (deliveryDate  < now.getDate())){
                       alert("送貨時間 '" + inputDate  + "' 不能小於今天  '" + now.getYear()+ "-" + (now.getMonth()+1) + "-" + (now.getDate()) + "' !");
                       obj.focus();
                       return false;
                    }
               } //es1
